			...
			<article id="2-<?= ++$exo; ?>">
				...
				<?php
					if (isset($_POST['check1'])) {
						echo "<p>vous avez fait le choix 1 : " . htmlspecialchars($_POST['check1']) . "</p>";
					}
					...
					<?php
						...
						<select name="selection" id="">
							<?php foreach($tableau as $row) {?>
								<option value="<?php echo htmlspecialchars($row["id"]);?>" <?php echo (isset($_POST['selection']) && $_POST['selection'] ==$row['id'] )? "selected" : ""; ?>> <?= htmlspecialchars($row["valeur"]);?></option>
							<?php } ?>
						</select>
					...
					<?php
						...
						if (isset($_FILES['upload2'])) {
							...
							if ( $extension == "jpeg" || $extension == "jpg" || $extension == "png" || $extension == "gif" ) {
								...
								$largeur_max_image = 500; //1600 * 900 zoom, 800 * 450 detail, 150 *150 crud, galerie 400*400
								...
								if ($extension == 'jpg' || $extension == 'jpeg') {
									...
									$image = imagecreatefromjpeg($fichier_temp);  //ouverture image
								...
								$nouvelle_image = imagecreatetruecolor($nouvelle_largeur_image, $nouvelle_hauteur_image); // creation image vide
